...
计费管理为用户资源分配和使用提供计量计费服务，需计费的资源均支持按时、按年、按月三种计费方式，支持资源的计费、扣费、续费及过期回收等订单管理操作，同时基于账户提供充值、扣费等交易管理。子账号共享主账号的账户余额，通过子账号创建的资源可直接通过共享余额进行扣费，并可通过主账号或子账号查看账户的交易流水及订单明细。

平台资源计费为预付费模式，仅容器集群部分功能为后付费模式，无论按时、按年、按月付费，在资源创建时都需保证账户余额可满足一个计费周期的扣费，下一个计费周期开始前即进行扣费。
...
账户余额不足下一个计费周期时，资源即会自动进入回收站，需要对资源账号及资源进行续费操作后，才可恢复使用；对于文件存储、对象存储、外网网卡、NAT 网关、VPN网关、负载均衡等资源，账户余额不足下一个计费周期时，资源会自动进行删除。
...
- 总费用指当前订单中所有计费资源一个计费周期的费用合计，如一个虚拟机订单中，包括指定的 CPU 内存、云盘（若有）、EIP（若有）等资源按照付费方式的费用总计。

点击立即购买后，即从账号余额扣除总计费用金额，并产生一个新购订单及一笔扣费的交易流水；若账号余额不足一个计费周期时，点击立即购买提示“租户的账号余额不足”，需要先对账号进行充值，才可进行购买和创建资源操作。
...
- 订单号：指当前订单的全局唯一标识符，且包含订单类型（新购、续费、升级、降级、退单及预付费六种）；
  - 新购是指用户新创建的计费资源，包括虚拟机、虚拟硬盘、弹性 IP、 外网网卡、文件存储、对象存储、NAT 网关、VPN 网关及负载均衡等产品；
...
租户可在帐号总览页面查看费用趋势，可通过选择所属租户查看某个租户在云平台近六个月内产生的交易信息，如下图所示：

![usercosttrend](../images/userguide/usercosttrend.png)

### 23.4.4 本月账单汇总

本月账单汇总从按产品汇总、按地域汇总、按项目组汇总、按订单类型汇总及按计费模式汇总五个方面用饼图展示，列表包括聚合类型、名称、总费用、平台费用及外部费用。
...
租户可从账单周期、所属产品、计费模式、所属地域、所属租户五个维度查看云平台的资源账单信息，列表包括资源ID、所属产品、所属项目组、计费模式、总费用、平台账户、外部账户、地域、租户ID、主账号名称及交易时间，如下图所示：

![userresourcebill](../images/userguide/userresourcebill.png)

- 资源 ID：账单的全局唯一标识符；
- 所属产品：云平台的产品，包括虚拟机、虚拟硬盘、外网 IP、VPN 网关、负载均衡、NAT 网关、网卡等；
- 所属项目：本次交易资源所绑定的项目；
- 计费模式：按小时、月、年的计费模式；
- 总费用：本次交易的总费用；
- 平台账户：本次交易消费平台账户的金额；
- 外部账户：本次交易消费外部账户的金额；
- 地域：资源所在的地域信息；
- 租户 ID：产生订单的租户信息及主账号邮箱；
- 主账号名称：充值的租户下的主账号名称；
- 交易时间：本次交易产生的时间。

### 23.4.6 导出资源账单

平台支持租户从账单周期、所属产品、计费模式、所属地域、所属租户五个维度筛选资源账单，并导出到本地 Excel 文件，为便平台运营管理和报表统计，如下图所示：

![userexportingresbill](../images/userguide/userexportingresbill.png)

### 23.4.7 账单明细

租户可从账单周期/所属产品/订单类型/计费模式/所属地域/所属项目六个维度查看云平台的账单明细，列表包括资源 ID、交易单号、订单号、所属产品、所属项目、计费模式、总费用、平台账户、外部账户、地域、租户 ID、主账号名称及交易时间，如下图所示：

![userbilldetails](../images/userguide/userbilldetails.png)

- 资源 ID：账单的全局唯一标识符；
- 交易单号：交易记录在云平台的唯一标识，同时展示交易类型（账户余额充值、免费账户充值及扣费三种）；
- 订单号：订单在云平台的唯一标识符，同时展示订单类型（升级和新购两种）；
- 所属产品：云平台的产品，包括虚拟机、虚拟硬盘、外网 IP、VPN 网关、负载均衡、NAT 网关、网卡等；
- 所属项目：本次交易资源所绑定的项目；
- 计费模式：按小时、月、年的计费模式；
- 总费用：本次交易的总费用；
