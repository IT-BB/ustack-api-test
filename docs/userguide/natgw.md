...
平台支持对 NAT 网关进行监控数据的收集和展示，通过监控数据展示每一个 NAT 网关的指标数据，同时支持为每一个监控指标设置阈值告警及通知策略。支持的监控指标包括CPU使用率、内存使用率、网络出/入带宽、网络出/包量及跟踪连接数。
...
- 计算集群：NAT 网关实例所在节点的计算集群类型，由平台管理员自定义，如 x86 机型和 ARM 机型，通过 ARM 机型创建的实例为 ARM 版 NAT 网关实例，已适配国产芯片、服务器及操作系统。
- 存储集群：NAT 网关实例的系统盘所属存储集群，可供选择，并展示存储用量。
- 机型：NAT网关支持单机版和主备版。
...
- 源地址类型：指当前 SNAT 规则的源地址类型，如 VPC 级别、子网级别、虚拟机级别。
...
用户可通过 NAT 网关详情【网络】标签页查看已绑定至 NAT 网关的外网 IP 地址列表及信息，包括IP、IP ID、IP版本、状态、带宽及操作项等，如下图所示：

![nateiplist](../images/userguide/nateiplist.png)

* IP ： 外网 IP 地址。
* IP ID ：当前已绑定至 NAT 网关的 EIP  全局唯一标识符。
* IP 版本 ：当前外网 IP 的IP版本，分为 IPv4 和 IPv6 。
...
支持用户开启删除保护功能，用于防止NAT产品被误删除释放，在创建时增加「删除保护」选项，如下图所示：

![natview1](../images/userguide/natview1.png)

删除资源时需要将此选项关闭才可以进行删除，如下图所示：

![natview2](../images/userguide/natview2.png)


## 12.16 开机关机

NAT网关 支持实例开关机操作，便于用户做离线操作，如下图所示：

![natview3](../images/userguide/natview3.png)

![natview4](../images/userguide/natview4.png)

注意事项：NAT网关关机后，相关联的所有SNAT、DNAT规则都会变为离线状态，NAT网关开机后，SNAT、DNAT规则会自动恢复运行
