...
定时器（Scheduler）是平台为用户提供自动化任务功能，可用于定期执行一系列任务的，如创建快照、一键巡检、资源用量统计等。可在指定的周期重复执行，也可仅执行一次，且每个任务支持多个资源批量操作。

* 支持定时创建快照，即实现硬盘的自动快照，同时支持为多个硬盘批量创建定时快照任务。
* 支持定时创建一键巡检、资源用量统计等公共定时任务。
...
用户可通过控制台直接创建一个用于执行创建快照的定时任务，用于虚拟硬盘的备份。创建定时任务时需要指定任务类型、重复周期、执行时间及关联资源，如下图所示：

![createjob](../images/userguide/createjob.png)

* 名称：定时任务的名称，用于标识定时任务。
* 保留数量：定期任务创建的资源，保留的资源数量，超过这个数量之前的资源，会被自动删除。
* 重复周期：支持单次、每天、每周、每月、间隔指定时间执行任务：
  * 单次：需要指定执行具体时间，24小时制；默认当天执行，若执行时间已过则为次日执行。
  * 每天：需要指定每天执行的整点时间，支持从 0 点到 23 点的 24 个整点，支持每个整点均会执行定时任务。
  * 每周：需要指定每次执行的日期和执行时间，支持从周一到周日的 0 点到 23 点，支持周一到周日按照每个执行时间进行定时任务的执行。
  * 每月：需要指定每次执行的日期和执行时间，支持从 1 号到 31 号或月末的 0 点到 23 点，同时每月的每一天每个执行时间进行定时任务的执行。
  * 间隔：需要指定间隔执行的间隔时间，支持按照间隔时间执行定时任务。
* 执行时间：任务具体执行的时间，根据重复周期不同，执行时间可支持不同的设置方法。
  * 当重复周期为单次时，可支持设置日期的具体时、分、秒。
...
![createjob](../images/userguide/createjob1.png)
...
![createjob](../images/userguide/createjob2.png)
...
- 执行时间：任务具体执行的时间，根据重复周期不同，执行时间可支持不同的设置方法。
- 时区：默认定时任务的时区东八区时间，即 GMT+8（北京时间）
- 资源用量周期：生成此时间范围内的用量报告，范围: 1~365，单位：天
- 所属地域：资源所在的地域
- 租户信息：选择租户，统计所选租户下的资源，支持多个
- 项目组信息：选择租户下的项目组，统计所选项目组的资源，支持多个
- 资源类型：资源的类型，包括：计算集群、存储集群、虚拟机
...
用户可通过控制台查看定时任务列表及相关信息，并可通过列表的名称进入定时任务详情查看定时任务的基本信息及任务执行记录，具体列表信息参考不同策略，创建时候填的项，下图仅做参考
...
用户可进入定时任务详情查看定时任务的详细信息，包括基本信息及任务执行记录信息，具体的更新操作，参考不同定时任务类型的创建流程：

![jobdetails](../images/userguide/jobdetails.png)

其中基本信息包括名称、ID、任务类型、重复周期、执行时间、创建时间等信息，而任务执行日志全面记录当前定时任务的任务执行计划及执行状态，包括资源 ID、开始时间、结束时间、任务状态及任务结果。
...
用户可在业务需要时对定时任务进行变更，支持变更定时任务的任务类型、重复周期、执行日期、执行时间及关联资源。定时任务更新后，不影响定时任务已执行的任务，执行新的任务类型时会根据新的定时策略进行操作。具体的更新操作，参考不同定时任务类型的创建流程。
