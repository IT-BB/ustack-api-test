...
EIP 资源支持独立申请和拥有，用户可通过控制台或 API 申请 IP 网段资源池中的 IP 地址，并支持将 IPv4 版本的 EIP 绑定至虚拟机、NAT 网关、负载均衡、VPN 网关、对象存储、文件存储、MySQL、Redis、Web 应用防火墙、智慧防火墙上，将 IPv6 版本的 EIP 绑定至虚拟机、负载均衡、对象存储上，为业务提供外网服务通道。
...
- 目前仅支持直通模式绑定 IPv6 的外网 IP 。
- 已绑定直通, 不支持同时绑定直通模式和 NAT 模式 EIP。

外网 IP 具有数据中心属性，仅支持绑定相同数据中心的虚拟资源。用户可通过平台自定义申请 EIP ，并对 EIP 进行绑定、解绑、调整带宽等相关操作。

## 9.2 申请外网 IP

申请 EIP 是指租户通过控制台从管理员自定义的外网 IP 网段中申请一个 IPv4 或 IPv6 的外网 IP 地址，并将 IP 地址绑定至虚拟机、负载均衡、NAT 网关、VPN 网关、文件存储、对象存储、MySQL、Redis 及安全产品等资源，为虚拟资源提供外网访问能力。

申请 EIP 时需指定 IP 版本、所属网段、IP 地址、资源名称及带宽上限等信息，可通过导航栏进入【外网 IP】资源控制台，通过“**创建**” 进入向导页面，如下图所示：
...
- 基本信息：外网 IP 地址的基本信息，包括名称、ID、IP 地址、IP 版本、带宽、绑定资源、状态、创建时间、过期时间及告警模板信息。

  - 可点击名称右侧按钮修改外网 IP 的名称和备注信息；
  - 可点击告警模板右侧按钮修改外网 IP 所关联的告警模板，默认不绑定告警模板。
- 监控信息：当前外网 IP 地址的监控信息，包括网卡出/入带宽、出/入包量及出/入带宽使用率。
- 操作日志：操作日志页面展示当前外网 IP 的操作日志。可提供自定义时间级别的日志展示，同时可对日志进行模糊搜索，默认提供两周内的操作日志，可通过切换日期周期查看不同时间周期的操作日志。

## 9.4 绑定外网 IP

绑定外网 IP 是指将 EIP 地址绑定至虚拟机、NAT 网关、负载均衡、VPN 网关、对象存储、文件存储、 MySQL 、 Redis ，为虚拟资源提供外网服务能力。

* 虚拟机支持绑定 50 个 IPv4 和 10 个 IPv6 外网 IP 地址，默认以第一个有默认路由的 IP 地址作为虚拟机的默认网络出口。
* 有默认路由的 IPv4 版本的 EIP 支持绑定的资源类型为虚拟机、负载均衡、NAT 网关、VPN 网关、对象存储、文件存储、MySQL、Redis、Web 应用防火墙、智慧防火墙。
* 无默认路由的 IPv4 版本的 EIP 支持绑定的资源类型为虚拟机、对象存储、文件存储、MySQL、Redis、Web 应用防火墙、智慧防火墙。
* IPv6 版本的 EIP 支持绑定的资源类型为虚拟机、负载均衡、对象存储。
...
* VPN 网关仅支持绑定一个外网 IPv4 且有默认路由的 IPv4 地址，不支持绑定 IPv6 外网 IP 地址。
* 负载均衡仅支持绑定有默认路由的外网 IP，包括 IPv4 和 IPv6 类型。

一个外网 IP 只能绑定一个虚拟资源，只有未绑定状态的外网 IP 支持绑定操作，且被绑定的资源须处于运行中、可用或关机状态。用户可通过外网 IP 资源列表操作项的“绑定”进入外网 IP 绑定向导页面，进行资源绑定操作，如下图所示：

![eiplink](../images/userguide/eiplink.png)

绑定时需选择被绑定资源的类型及绑定资源对象：

（1）资源类型：指被绑定对象的资源类型，支持绑定给虚拟机、负载均衡、NAT 网关、VPN 网关、对象存储、文件存储、MySQL、Redis、Web 应用防火墙、智慧防火墙。

（2）资源对象：指被绑定的资源对象，不同的资源类型可选的资源对象不同。

* 虚拟机：可根据虚拟机名称及内网 IP 地址选择需绑定的虚拟机资源，不可选择至已绑定 50 个 IPv4 或 10 个 IPv6 地址的虚拟机；支持无 QGA 虚拟机绑定外网 IP ，但实际 IP 下发需手动进入虚拟机配置。
* 负载均衡：可根据名称和 ID 选择需绑定的负载均衡资源，仅支持选择未绑定外网 IP 地址且类型为外网的负载均衡实例，支持为负载均衡绑定 IPv4 或 IPv6 外网 IP。
* NAT 网关：可根据名称和 ID 选择需绑定的 NAT 网关资源，仅支持绑定 IPv4 版本且有默认路由的外网 IP。
* VPN 网关：可根据名称和 ID 选择需绑定的 VPN 网关资源，仅支持绑定 IPv4 版本且有默认路由的外网 IP。
* 文件存储：可根据名称和 ID 选择需绑定的文件存储资源，仅支持绑定 IPv4 版本的外网 IP。
* 对象存储：可根据名称和 ID 选择需绑定的对象存储资源，支持绑定 IPv4 或 IPv6 版本的外网 IP。
* MySQL：可根据名称和 ID 选择需绑定的 MySQL 资源，仅支持绑定 IPv4 版本的外网 IP。
* Redis：可根据名称和 ID 选择需绑定的 Redis 资源，仅支持绑定 IPv4 版本的外网 IP。
...
- 按年，按月付费的弹性IP，降级带宽，即时生效，下个付费周期按新配置扣费
...
虚拟机绑定 EIP 支持两种 IP 类型，分别为直通模式 EIP 和 NAT 模式 EIP ，如图所示：

![vpc-nat](../images/userguide/vpc-nat.png)

- NAT-EIP 实际带宽值为内网带宽、 EIP 带宽二者中较小带宽值
- NAT-EIP 安全组规则受虚拟机内网安全组、外网安全组共同影响，仅生效内、外网安全组相同的规则。若内、外网安全组规则互斥，则 NAT-EIP 拒绝所有流量
