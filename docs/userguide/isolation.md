...
通过导航栏进入隔离组页面可查看当前账户下隔离组资源的列表及相关详细信息，包括隔离组名称、资源 ID、计算集群 ID、策略对象类型、状态、主机数、策略对象、策略、执行与启用、所属租户、创建时间及操作项，如下图所示：
...
- 策略：目前隔离组支持亲和以及反亲和策略；
- 执行与启用：强制执行开启后，如果节点不能满足虚拟机调度策略，虚拟机实例将会一直处于调度中状态，直到调度策略条件满足；是否启用隔离组；
- 所属租户：隔离组所属租户；
- 创建时间：隔离组的创建时间；
- 操作项：对隔离组的创建、修改、禁用、删除。
...
隔离组的基本信息，包括名称、资源 ID、计算集群、计算集群 ID、策略对象类型、策略、策略对象、实例数量、强制执行、是否启用、所属租户、状态。
...
支持将关机/断电/运行中状态下且与隔离组所属计算集群一致的的虚拟机加入隔离组，运行中状态的虚拟机加入隔离组后需要关机/断电后重新启动生效，如下图所示：
...
隔离组为禁用状态且已绑定策略对象时，可操作启用隔离组，如下图所示：

![enableisolation](../images/userguide/enableisolation.png)

## 28.8 禁用隔离组

隔离组为启用状态时，可操作禁用隔离组。禁用隔离组时，隔离组内实例会被移除，且不允许操作加入实例。如下图所示：

![disableisolation](../images/userguide/disableisolation.png)
...
管理员支持创建节点隔离组，策略对象类型为节点组，支持亲和，反亲和策略。创建节点组启用强制执行可能会导致虚拟机无法调度。如下图所示：

![deleteisolation](../images/userguide/createnodeisolation.png)

### 28.11.2 加入和移除实例

管理员支持在节点隔离组中加入/移除虚拟机实例，管理员支持为租户创建虚拟机时选择节点隔离组。处于关机/断电/运行中状态的虚拟机实例可以加入到隔离组中，运行中状态的虚拟机加入隔离组后需要关机/断电后重新启动生效，如下图所示：

![nodeaddinstance](../images/userguide/nodeaddinstance.png)
![nodedelinstance](../images/userguide/nodedelinstance.png)

### 28.11.3 加入和移除目的节点

管理员支持将处于可用状态的节点加入到目的节点组中或者从节点组中移除目的节点。如下图所示：

![adddesnode](../images/userguide/adddesnode.png)
![deldesnode](../images/userguide/deldesnode.png)
