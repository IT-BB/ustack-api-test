...
- VPC 到第三方私有云内网的连接：通过 IPsecVPN 服务将第三方私有云的 VPC 私有网络和 本平台 VPC 网络的虚拟资源进行连接，构建多云混合服务模式。
...
创建 VPN 网关时需指定机型、VPC 网络、子网、外网 IP、安全组及 VPN 网关名称和备注信息，可通过导航栏进入【 VPN 网关】资源控制台，通过“创建 VPN 网关”进入创建向导页面，如下图所示：
...
- 计算集群：VPN 网关实例所在节点的计算集群类型，由平台管理员自定义，如 x86 机型和 ARM 机型，通过 ARM 机型创建的实例为 ARM 版 IPsecVPN 网关实例，已适配国产芯片、服务器及操作系统。
- 存储集群：VPN 网关实例的系统盘所属存储集群，可供选择，并展示存储用量。
- 机型：VPN网关支持单机版和主备版。
...
1. 选择并配置以上信息后，可选择购买数量和付费方式，确认订单金额并点击“立即购买” 进行 VPN 网关创建：
...
VPN 网关实例相关的监控图表及信息，包括CPU使用率、内存使用率、网卡入/出带宽、网卡入/出包量及 VPN 网关的出带宽使用率，支持查看 1 小时、6 小时、12 小时、1 天及自定义时间的监控数据。
...
用户可通过控制台或 API 的方式删除不需要的 VPN 网关实例，删除时会自动解绑已绑定的外网 IP 地址。支持删除任何状态的VPN 网关，删除后会将 VPN 网关已关联的隧道删除。
...
支持用户开启删除保护功能，用于防止VPN产品被误删除释放，在创建时增加「删除保护」选项，如下图所示：

![vpnview1](../images/userguide/vpnview1.png)

删除资源时需要将此选项关闭才可以进行删除，如下图所示：

![vpnview2](../images/userguide/vpnview2.png)

### 13.3.10 开机关机

VPN网关 支持实例开关机操作，便于用户做离线操作，如下图所示：

![vpnview3](../images/userguide/vpnview3.png)

![vpnview4](../images/userguide/vpnview4.png)

注意事项：VPN网关关机后，相关联的所有隧道都会变为离线状态，VPN网关开机后，隧道通信会自动恢复运行
...
创建对端网关需指定对端网关的公网 IP 地址。由于 **IPSecVPN 服务是基于互联网的加密通信服务，在使用前需确认两端网关均有固定的互联网 IP 地址。**用户在输入正确定的 IP 地址后即可创建对端网关，用于创建隧道连接。
...
用户可通过控制台或 API 的方式删除不需要的对端网关实例，支持删除任何状态的对端网关，删除后会将对端网关已关联的隧道删除。
...
![createtunnel3](../images/userguide/createtunnel3.png)
...
![tunnelmonitor](../images/userguide/tunnelmonitor1.png)
...
远端 VPN 设备或网络平台即本端 私有云平台标记的对端网关，可以是物理硬件或和软件系统，如路由器、防火墙、VPN 设备或使用 `OpenSwan`和 `StrongSwan`搭建在 Linux 系统上的 VPN 服务器系统；同样也可以是其它云平台的 VPN 服务，如 第三方公有云 VPN 网关服务或阿里云的 IPSecVPN 连接等。

为演示方便，本文主要通过以下几种示例环境与 私有云平台 IPsecVPN 网关建立连接：
...
通过在 第三方公有云平台 和 私有云平台之间建立 IPSecVPN 连接，实现私有云和公有云混合构建及数据传输。

本文描述在私有云和公有云间建立基于 IKEv1 版本的 IPSecVPN 连接。
...
> 本文假设已在私有云平台上部署 VPN 网关和对端网关，并已通过以上配置参数创建 VPN 隧道，等待 第三方公有云配置好 VPN 隧道后，即可进行 VPN 连接。

#### 13.6.1.2 配置公有云网关

第三方公有云 IPSecVPN 服务与 私有云平台上的 VPN 服务的配置过程相同，均需创建 VPN 网关、客户网关，并建立配置 VPN 隧道进行 VPN 连接。

确保配置前已创建 VPC 网络的子网为 10.23.0.0/16 和 10.25.0.0/16 ，并已在 VPC 内创建云主机 10.23.228.173。

1. 使用 113.31.115.114 外网 IP 地址创建 VPN 网关，如下图所示：

   ![vpngw](../images/userguide/vpngw.png)
2. 使用 私有云平台侧 VPN 网关的公网 IP 地址创建客户网关，本示例假设 私有云平台环境 VPN 网关的出口 IP 为固定公网 IP 地址，如下图所示：

   ![remotegw](../images/userguide/remotegw.png)
3. 使用已创建的 VPN 网关和客户网关，采用前提条件中的 IKE 和 IPSec 策略创建 VPN 隧道，如下图所示：

   ![tunnelinfo](../images/userguide/tunnelinfo.png)

   * 本端网段和对端网段与 私有云平台侧隧道正好相反，私有云平台侧隧道配置的本端网段为 `10.0.192.0/20` ，对端网段为 `10.23.0.0/16 ` 和 `10.25.0.0/16`  。
   * 本端 ID 和对端 ID 即对应 私有云平台侧的本端标识和对端标识，如图所示与 私有云平台侧的配置正好相反，私有云平台侧配置的本端标识为 `106.75.234.78` ，对端标识为 `113.31.115.114` 。
   * IKE 策略的版本、加密算法、认证算法、预共享密钥 、DH 组 均与 私有云平台侧保持一致。
   * IPSec 策略安全协议、加密算法、认证算法、PFS DH 组与 私有云平台侧保持 一致。
4. 分别查看 私有云平台侧和 第三方公有云侧的 VPN 隧道连接状态，等待隧道自动连接。私有云平台侧可通过列表上连接状态直接查看隧道是否已连接，第三方公有云侧需进入隧道详情页面查看"VPN 隧道状态"的监控，如下图所示：

   ![tunnelmonitor](../images/userguide/tunnelmonitor.png)
5. 在 第三方公有云的隧道监控中查看 VPN 隧道状态已变为 1 ，代表 VPN 已连接，同时在 私有云平台 中隧道的连接状态流转为“已连接” ，如下图所示：

   ![tunnelstatus1](../images/userguide/tunnelstatus1.png)

#### 13.6.1.3 配置验证

在已连接状态时，私有云平台 侧会自动下发对端网段为目标地址的路由至本端网段内的虚拟机中，可登入提前准备的本端虚拟机查看相关网络及路由配置信息。

如下图所示，本端虚拟机的 IP 地址为 `10.0.192.32` ，下发的路由为 `10.23.0.0/16`  及 `10.25.0.0/16` ，即代表虚拟机可与 第三方公有云侧的两个网段进行通信。

![vpnvmroute](../images/userguide/vpnvmroute.png)

可通过 ping 命令检测与 第三方公有云虚拟机的网络连通性，如下图代表两端内网的虚拟机网络互通。

![vpnping](../images/userguide/vpnping.png)

根据以上的配置过程，即可通过 IPSecVPN 的方式将 私有云平台 和与 第三方公有云内网打通。

### 13.6.2 Cisco 防火墙配置

通过在 IDC 数据中心的 Cisco 防火墙与 私有云平台 之间建立  IPSecVPN 连接，实现私有云和 IDC 数据中心网络互通和数据交互。
...
> 本文假设已在 私有云上部署 VPN 网关和对端网关，并已通过以上配置参数创建 VPN 隧道，等待数据中心的 Cisco 防火墙配置好 VPN 隧道后，即可进行 VPN 连接。
...
通过在任意有公网 IP 地址的 Linux 主机上安装并配置 StrongSwan 与 私有云平台之间建立  IPSecVPN 连接，实现私有云和安装 IPSec 软件的主机对接，使相同网段的客户主机通过 IPSec 主机与 私有云平台虚拟机进行通信。

#### 13.6.3.1 前提条件

在建立 IPSecVPN 连接进行通信前，需确认两端要建立 IPSecVPN 连接的网络拓扑关系及配置参数信息。

| 网络配置和配置参数   | 私有云                          | IDC 侧 StrongSwan                   |
|----------------------|---------------------------------|-------------------------------------|
...
> 本文假设已在 私有云上部署 VPN 网关和对端网关，并已通过以上配置参数创建 VPN 隧道，等待数据中心的 StrongSwan 配置好 VPN 隧道后，即可进行 VPN 连接。
...
2. 在 IDC 数据中心 StrongSwan下 `10.23.0.0/16` 网段的主机内添加到达 私有云平台 侧网段的路由，使两端主机可以互相通信。
...
通过 VPN 网关将 私有云平台建立 VPC 到 VPC 的 VPN 连接，实现两个 VPC 内虚拟机互问及数据传输入。
...
| 网络配置和配置参数   | 私有云 VPC1                     | 私有云 VPC2                      |
|----------------------|---------------------------------|----------------------------------|
| VPN 网关公网 IP 地址 | 192.168.178.169                 | 192.168.178.168                  |
| VPC 网段             | 10.0.0.0/16                     | 172.16.0.0/16                    |
| 客户虚拟机 IP        | 10.0.0.11                       | 172.16.0.4                       |
| 预共享密钥           | test.1231                       | test.1231                        |
| IKE 版本             | V2                              | V2                               |
| IKE 策略             | 认证 SHA1、加密 AES128、DH 组 2 | 认证 SHA1、加密 AES128、DH 组 2  |
| IPSec 安全传输协议   | ESP                             | ESP                              |
| IPSec 策略           | 认证 SHA1、加密 AES128、PFSDH 2 | 认证 SHA1、加密 AES128、PFS DH 2 |
...
1. 分别在 VPC1 和 VPC2 中创建 VPN 网关，并确认两个网关地址，如下图所示；

   ![vpngwlist2](../images/userguide/vpngwlist2.png)
2. 分别针对两个 VPN 网关创建对应的对端网关，如下图所示：
...
* VPC1 的客户虚拟机 IP 地址为 `10.0.0.11` ，下发的目标路由为 `172.16.0.0/16`
* VPC2 的客户虚拟机 IP 地址为 `172.16.0.4` ，下发的目标路由为 `10.0.0.0/16`
