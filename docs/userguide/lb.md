...
平台负载均衡为用户提供业务级别的高可用方案，可以将业务应用同时部署至多个虚拟机中，通过负载均衡和DNS域名的方案设置流量均衡转发，实现多业务级别的流量负载均衡。

当大并发流量通过负载均衡访问虚拟机业务时，可通过最小连接数、加权轮询等算法，将请求转发给后端最健壮的虚拟机进行处理，请通过负载均衡将请求结果返回给客户端，保证业务可用性和可靠性。

同时用户可通过智能DNS服务，将两个数据中心的负载均衡实例同时绑定至一个域名，使用DNS实现跨数据中心的业务容灾方案。
...
- 支持 SSL 证书认证：针对 HTTPS 协议，提供统一的证书管理服务和 **`SSL Offloading `** 能力，并支持 HTTPS 证书的单向和双向认证。SSL 证书部署至负载均衡，仅在负载均衡上进行解密认证处理，无需上传证书到后端业务服务器，降低后端服务器的性能开销。
...
- 监控数据：负载均衡级别提供CPU使用率、内存使用率、TCP连接数、每秒网卡出/入流量、每秒网卡出/入包数量的监控及告警；VServer 级别提供连接数、HTTP 2XX、HTTP 3XX、HTTP 4XX、HTTP 5XX 等监控数据及告警。
...
用户在平台创建负载均衡需指定计算集群、存储集群、CPU、机型、网络类型、VPC 网络、子网、外网 IP 及安全组等信息，可通过导航栏进入【负载均衡】资源控制台，通过**“创建负载均衡”**进入向导页面，如下图所示：
...
- 计算集群：支持选择计算集群，并展示所选计算集群的 CPU 使用率和内存用量。计算集群由平台管理员自定义，如 x86 机型和 ARM 机型，通过 ARM 机型创建的实例为 ARM 版负载均衡实例，已适配国产芯片、服务器及操作系统。
- 存储集群：支持选择系统盘存储集群，并展示所选存储集群的存储用量。
...
- 外网 IP ：当网络类型为外网时，可配置负载均衡实例自动绑定的外网 IP 地址，仅支持绑定有默认路由的外网 IP 地址作为负载均衡的入口地址，支持绑定 IPv4 和 IPv6。
- 安全组：当网络类型为外网时，可配置负载均衡自动绑定的外网安全组，用于外网访问负载均衡的安全控制。
- 实例名称/备注：负载均衡实例的名称及备注信息。
- 项目组：设置实例所属项目组，默认为 default。
- 标签：选择对应的资源标签，便于管理。

2. 选择购买数量和付费方式，确认订单金额并点击“立即购买” 进行负载均衡实例的创建。

- 购买数量：按照所选配置及参数创建负载均衡实例，一次仅支持创建 1 个负载均衡实例；
...
负载均衡列表可查看当前账户下所有负载均衡资源信息，包括名称、资源 ID、机型、计算集群类型、存储集群类型、IP、VPC、子网 ID、安全组 ID、VServer 数量、创建时间、过期时间、计费方式、状态及操作项，如下图所示：

![lblist](../images/userguide/lblist.png)

- 名称/ID：负载均衡的名称及全局唯一标识符。
- 机型：负载均衡实例的机型，有单机版和主备版两种。
- 计算集群类型：负载均衡实例所在的计算集群类型。
- 存储集群类型：负载均衡实例所在的存储集群类型。
- IP：负载均衡对外提供服务的访问地址，网络类型为内网时为所属子网自动分配的 IP 地址，网络类型为外网时为所绑定的外网 IP 地址。
- VPC：负载均衡实例所在的 VPC 网络。
- 子网 ID：负载均衡实例所在的子网。
- 安全组 ID：负载均衡实例绑定的安全组 ID。
- VServer 数量：负载均衡实例上已创建的监听器 VServer 数量。
- 创建时间/过期时间：负载均衡的创建时间及费用过期时间。
- 计费方式：负载均衡创建时指定的计费方式。
- 状态：负载均衡的的运行状态，包括创建中、运行中、删除中等。

列表上操作项是指对单个负载均衡实例的操作，包括绑定/解绑外网 IP、配置升级、升级机型、日志管理、续费、修改安全组、修改告警模板、修改标签、删除等，可通过搜索框对负载均衡资源列表进行搜索和筛选，支持模糊搜索。
...
负载均衡器的基本信息，包括资源 ID、资源名称、计算集群类型、存储集群类型、状态、创建时间、过期时间、计费方式、内网/外网 IP、告警模板及日志管理信息，可点击资源名称右侧按钮修改资源名称及备注，可点击告警模板右侧按钮修改负载均衡所关联的告警模板，可点击日志管理右侧按钮选择存储位置。

**（2）监控信息**

负载均衡实例相关的监控图表及信息，包括 TCP 连接数、每秒网卡出/入流量及出/入包数量，支持查看 1 小时、3 小时、6 小时、12 小时、1 天、3 天、7 天及自定义时间的监控数据。
...
当前负载均衡的监听器生命周期管理，包括 VServer 的添加、查看、修改、删除操作管理，同时还可对 VServer 的后端服务节点及七层内容转发规则进行管理，详见 [VServer 管理](#_113-VServer-管理)。负载均衡支持通过系统全局配置管理 VServer 的数量上限，方便管理员根据业务需求灵活调整负载均衡实例的容量限制，提高资源利用效率，满足多场景的部署需求。

**（5）访问日志**

访问日志页面支持实时查看主备机器七层负载均衡访问日志，最多支持在线查看100M日志。日志信息支持下载功能。

**（5）操作日志**

操作日志页面可查看负载均衡的操作信息，支持查看 1 小时及自定义时间的日志信息，最长可查询 12 个月的操作日志信息。具体信息包括操作（API）名称、所属模块、地域、关联资源、操作者、操作结果及操作时间。

**（6）事件信息**

事件页面会记录负载均衡资源的部分核心操作事件，提供事件详细记录，用户可通过事件描述定位问题，包括事件类型、事件等级、事件内容、事件发生次数、开始事件和更新时间。
...
负载均衡支持进行访问日志管理，仅支持 7 层访问日志，转储周期为 5 分钟；开启日志管理功能选定的对象存储需要与负载均衡在同一个 VPC 下，对象存储日志默认保留 6 个月，过期会自动删除；负载均衡日志路径为 lb-log/lb-xxxx/2022/09/access.log-20220923185657-bhetvdnj.gz。
...
支持用户在线配置负载均衡的 CPU 规格，负载均衡运行中即可完成 CPU 规格升级。

![updatelbconfig](../images/userguide/updatelbconfig.png)

平台支持 负载均衡 实例离线配置升降级，实例关机后，可执行CPU规格升级或降级操作。

![updatelbconfig2](../images/userguide/updatelbconfig2.png)
...
支持用户开启删除保护功能，用于防止负载均衡产品被误删除释放，在创建时增加「删除保护」选项，如下图所示：

![lbview1](../images/userguide/lbview1.png)

删除资源时需要将此选项关闭才可以进行删除，如下图所示：

![lbview2](../images/userguide/lbview2.png)

### 11.2.14 开机关机

负载均衡 支持实例开关机操作，便于用户做离线配置升降级等操作，如下图所示：

![lb1](../images/userguide/lb1.png)

![lb2](../images/userguide/lb2.png)

注意事项：

* 负载均衡关机后，相关联的所有VServer都会变为离线状态，负载均衡开机后，VServer会自动恢复运行
* 负载均衡关机后，相关联的所有VServer都会变为离线状态，并且和VServer绑定的水平伸缩，也将同步离线；负载均衡开机后，VServer会自动恢复运行,绑定的水平伸缩策略也将自动恢复运行。
...
* 端口：负载均衡业务对外或对内提供服务时用来接收请求的应用端口，端口范围为 `1~65535` ，本示例使用 23 端口，用于提供高可用的 Telnet 服务。
  ...
  > 323，9102，9103，9104，9105，9106，60909，60910 等端口被占用，在任何协议下均不可使用。
  >
...
用户为负载均衡实例创建一个基于 HTTP 协议的监听器，提供基于 HTTP 协议的负载均衡业务服务，本文以创建 `HTTP:8080` （WEB）服务为例进行创建。HTTP 协议下平台默认支持 WebSocket，负载均衡支持 WebSocket 长连接的代理转发，确保客户端与后端服务器之间的双向通信保持稳定。用户可通过负载均衡详情页面 VServer 左侧导航栏的【添加】进入 VServer 监听器的创建向导页面，如下图所示：

![httpvs](../images/userguide/httpvs.png)

根据向导页面配置 VServer 监听器，包括协议、端口、负载均衡算法、重定向开关按键、重定向至、会话保持、连接空闲超时及健康检查：

* 监听协议：负载均衡业务的网络协议，支持 TCP、UDP、HTTP、HTTPS ，本示例中选择 HTTP 。
* 端口：负载均衡业务对外或对内提供服务时用来接收请求的应用端口，端口范围为 `1~65535` ，本示例使用 8080 端口，用于提供基于 HTTP 协议的高可用 WEB 服务。
...
* 会话保持：针对 HTTP 和 HTTPS 协议，为“加权轮询”的负载均衡算法提供 Cookie 植入方式的会话保持功能，支持自动生成KEY 和自定义 KEY。选择自动生成 KEY 则由平台自动生成 Key 进行植入，选择自定义 Key 时需输入 KEY 值（只能输入数字、字母及_字符）。
* 连接空闲超时：客户端至负载均衡的连接空闲超时限制，范围为 `1~86400` 秒。默认值为 60 秒，即在 60 秒内客户端对负载均衡一直无访问请求，平台会自动中断连接。
...
用户为负载均衡实例创建一个基于 HTTPS 协议的监听器，提供基于 HTTPS 协议的负载均衡业务服务，本文以创建基于 SSL 证书加密的 `HTTPs:443` （WEB）服务为例进行创建。HTTPS 协议下平台默认支持 WebSocket，负载均衡支持 WebSocket 长连接的代理转发，确保客户端与后端服务器之间的双向通信保持稳定。用户可通过负载均衡详情页面 VServer 左侧导航栏的【添加】进入 VServer 监听器的创建向导页面，如下图所示：

![httpsvs.png](../images/userguide/httpsvs.png)
...
* 后端协议：负载均衡的 HTTPS 监听器下，后端支持 HTTP、HTTPs 两种协议。

  * 在 HTTP 协议下，LB 解密流量后以 HTTP 明文转发至后端，降低后端计算开销；
  * 在 HTTPs 协议下，LB 透传加密流量至后端，需后端服务器配置有效证书（支持端到端加密）。
* 负载均衡算法：负载均衡分发请求到后端 RealServer 的调度计算策略，支持加权轮询、最小连接数和基于源地址三种算法。
* 会话保持：针对 HTTP 和 HTTPS 协议，为“加权轮询”的负载均衡算法提供 Cookie 植入方式的会话保持功能，支持自动生成KEY 和自定义 KEY。选择自动生成 KEY 则由平台自动生成 Key 进行植入，选择自定义 Key 时需输入 KEY 值（只能输入数字、字母及_字符）。
* 连接空闲超时：客户端至负载均衡的连接空闲超时限制，范围为 `1~86400` 秒。默认值为 60 秒，即在 60 秒内客户端对负载均衡一直无访问请求，平台会自动中断连接。
* 健康检查：根据规则对后端服务节点进行健壮性检查，可自动检测并隔离服务不可用的后端服务节点。HTTP协议端检查和 HTTP 检查两种方式，其中 HTTP 检查支持按 URL 路径和请求 HOST 头中携带的域名进行健康检查，筛选健康节点。
...
- 状态：VServer 监听器的服务状态，包括绿色、黄色和红色；
  - 绿色：VServer 中添加的所有服务节点的健康状态均为正常；开启“重定向”的HTTP协议的 VServer 默认为绿色。
...
VServer 的基本信息，包括资源 ID、协议端口、负载均衡算法、会话保持、会话保持 Key、连接空闲超时、健康检查方式、节点状态、VS 状态、告警模板及创建时间等信息。若 VServer 监听协议为 HTTP/HTTPS ，可查看 HTTP 健康检查路径、HTTP 检查域名、SSL 解析模式、服务器证书及客户端证书等信息。
...
- 服务节点：VServer 的服务节点生命周期管理，包括服务节点的添加、查看、修改、启用、禁用及删除等，详见[服务节点管理](#_114-服务节点管理)。
...
通过 VServer 详情页面的"服务节点"标签页，可查看 VServer 监听器后端已添加的服务节点资源列表信息，包括服务节点、资源 ID、内网 IP、端口、权重、节点模式、节点状态及操作项，如下图所示：

![rslist](../images/userguide/rslist.png)

- 服务节点：当前服务节点的全局 RS 唯一标识符。
- 资源 ID：当前服务节点已绑定的虚拟机 ID。
...
负载均衡支持实时查看主备机器七层负载均衡访问日志，如下图所示：

![accesslog.png](../images/userguide/accesslog.png)

- 主、备机器切换时，会清空日志信息；
- 访问日志是实时在线查看日志，退出页面再次查看的时候会保存16KB的日志；
- 访问日志最多支持在线查看100M日志；
- 访问日志支持下载功能；
...
![CreateServerSSL1.png](../images/userguide/CreateServerSSL1.png)
