...
通过导航栏进入虚拟机控制台，切换至镜像管理页面可查看当前账户下自制镜像资源以及ISO镜像资源的列表及相关详细信息，包括镜像名称、资源 ID、状态、是否加密、特性支持、系统类型、操作系统、操作系统、镜像格式、镜像大小、引导方式及操作项，如下图所示：
...
- 镜像格式：指描述自制镜像所采用的格式，例如ISO、QCOW2 等；
- 镜像大小：指明自制镜像的磁盘占用空间，方便用户了解存储需求；
- 引导方式：标明自制镜像的引导方式，是通过 BIOS 还是 UEFI 引导；
- 项目组：指当前自制镜像所在的项目组信息，HCI、Virt 版不支持查看项目组；
- 操作：对单个自制镜像的操作，包括从镜像创建虚拟机、下载、取消上传、修改、及删除操作；
...
![selfimage2.png](../images/userguide/selfimage2.png)

使用自制镜像和ISO镜像创建虚拟机的过程与基础镜像相同，可根据提示进行操作。从镜像创建虚拟机时设置的管理员密码会覆盖原镜像操作系统中的密码，需使用新密码登录创建的虚拟机。

> 已加密镜像创建虚拟机暂不支持修改密钥。

![selfimage](../images/userguide/flatnetwork.png)

![selfimage](../images/userguide/vpcNetwork.png)

在云平台的标准环境和 HCI、Virt 环境下，虚拟机的网络设置存在一些区别。在HCI 和 Virt 版本，平台在原理上精简了设计，不包含VPC、子网、外网IP、外网安全组等选项，创建虚拟机时可以选择不同的扁平网络，如上分别是标准环境和 HCI 与 Virt 环境创建虚拟机时网络设置。

## 4.3 导入镜像

导入镜像是指租户或平台管理员将第三方业务虚拟机以镜像的方式迁移到平台镜像仓库，使租户可以在通过导入的镜像创建并部署业务虚拟机，是用户将业务迁移的重要通道。

支持用户导入 Linux 和 Windows 发行版及自定义镜像，并支持 X86 架构和 aarch64 两种系统架构镜像的导入；云平台的镜像格式默认为 RAW ，上传ISO格式镜像在ISO镜像管理界面上传，用户上传 VHD、VMDK、QCOW2、OVA等格式的镜像时，需先将镜像转换为 QCOW2 格式的镜像才可导入，有关转换镜像及自定义镜像的具体操作可参考自制镜像列表上展示的[自定义镜像指南](/CloudDocs_v2.x/customimage/README.md) 。
...
* 系统架构：导入镜像的系统架构，包括 `x86_64` 、loongarch64 和 `aarch64` ，需根据导入镜像进行选择；
* 引导方式：导入镜像的引导方式，支持 `BIOS`,`UEFI`;
* 系统平台：指导入镜像的操作系统平台；
  * Linux 操作系统的系统平台包括 Centos 、 Ubuntu、UOS、Kylin V10、Debian、TencentOS、SUSE、Rocky、OpenEuler、Anolis ；
  * Windows 操作系统的系统平台仅支持 Windows ；
* 系统版本：当前需导入镜像的操作系统版本；
  * CentOS x86_64 架构支持  `6.5~6.10` 、 `7.0~7.9` 、`8.3` 和 `8.5`版本；
  * CentOS aarch64 架构支持 `7.6~7.9` 、 `8.3` 和 `8.5`版本；
  * Ubuntu x86_64 架构支持 `14.04` 、`16.04` 、`18.04` 、`20.04` 、`22.04` 、和 `20.10` 版本；
  * Ubuntu aarch64 架构支持 `16.04` 、`18.04` 、`20.04` 、`22.04` 版本；
  * Debian x86_64 架构支持 `10.12`版本；
  * Debian aarch64 架构支持 `10.12`版本；
  * UOS x86 架构支持 `20`版本；
  * KylinV10 x86_64 架构支持 `SP1`版本；
  * KylinV10  aarch64 架构支持 `SP1`、`SP2` 版本；
  * KylinV10  loogarch64 架构支持 `SP1` 版本；
  * TencentOS x86_64 架构支持 `2.4` 版本；
  * SUSE x86_64 架构支持 `12` 版本；
  * Rocky x86_64 架构支持 `9.2`、`9.4` 版本；
  * OpenEuler x86_64 架构支持 `22.03_SP4` 版本；
  * OpenEuler aarch64 架构支持 `22.03_SP4` 版本；
  * Anolis x86_64 架构支持 `8.9` 版本；
  * Windows 支持 `7` 、`10` 、`2008` 、`2008 r2` 、`2012` 、`2012 r2` 、`2016` 、`2019` 、`2022` 版本。
* agent支持：仅在自制镜像导入时出现，当前导入镜像支持的特性，包括cloud-init、qemu-ga；
* 标签：支持导入的镜像选择绑定标签。
* 项目组：指自制镜像的项目组信息，HCI、Virt 版不支持选择项目组；
...
支持用户对自制镜像和ISO镜像操作修改，修改项包括操作系统、系统架构、引导方式、系统平台、系统版本和agent支持，如下图所示：

![editimage](../images/userguide/editimage.png)

## 4.6 镜像上传列表

用户通过导入本地文件上传镜像时，在导入镜像弹框展示上传镜像进度条，如下图所示：

![uploadimage](../images/userguide/uploadimage1.png)
...
![uploadimage](../images/userguide/uploadimage2.png)

![uploadimage](../images/userguide/uploadimage3.png)
