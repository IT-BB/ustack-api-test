...
安全组仅可作用于**同一个数据中心** 内具有相同安全需求的虚拟机、负载均衡、NAT 网关、网卡、对象存储、文件存储、MySQL 及 Redis，工作原理如下图所示：

![Secrity_group](../images/userguide/Secrity_group.png)

安全组具有独立的生命周期，可以将安全组与虚拟机、负载均衡、NAT 网关、网卡、对象存储、文件存储、MySQL、Redis 绑定在一起，提供安全访问控制，与之绑定的虚拟资源销毁后，安全组将自动解绑。

- 安全组对虚拟机的安全防护针对的是一块网卡，即安全组是与虚拟机的默认虚拟网卡或网卡绑定在一起，分别设置访问控制规则，限制每块网卡的出入网络流量；
- 如安全组原理图所示，安全组与提供外网 IP 服务的虚拟外网网卡绑定，通过添加出入站规则，对南北向（虚拟机外网）的访问流量进行过滤；
- 安全组与提供私有网络服务的虚拟网卡或网卡绑定，通过添加出入站规则，控制东西向（虚拟机间及网卡间）网络访问；
...
- 一个安全组支持同时绑定至多个虚拟机、负载均衡、NAT 网关、网卡、对象存储、文件存储、MySQL 及 Redis 实例；
- 虚拟机支持绑定一个内网安全组和一个外网安全组，分别对应虚拟机默认的内网网卡和外网网卡上，其中外网安全组对绑定至虚拟机的所有外网 IP 地址生效；
- 网卡仅支持绑定一个安全组，与虚拟机默认网卡绑定的安全组相互独立，分别限制对应网卡的流量；
- 外网负载均衡和 NAT 网关实例仅支持绑定一个安全组，可更换安全组应用不同的网络访问规则；
- 对象存储、文件存储、MySQL 和 Redis 仅支持有外网时绑定安全组，且仅支持绑定一个安全组，可更换安全组应用不同的网络访问规则；
- HCI 和 Virt 版本仅支持虚拟机绑定安全组。

创建虚拟机时支持指定内网/外网安全组，允许随时修改安全组的出入站规则，新规则生成时立即生效，可根据需求调整安全组出/入方向的规则。支持安全组全生命周期管理，包括安全组创建、修改、删除及安全组规则的创建、修改、删除等生命周期管理。
...
- 基本信息：当前安全组的基本信息，包括资源ID、资源名称、规则数量、绑定资源数量及创建时间等信息。
...
如上图列表所示，已绑定资源的列表信息包括资源名称、资源ID、资源类型、网卡类型等信息，其中资源类型包括虚拟机、负载均衡、 NAT 网关、网卡、对象存储、文件存储、MySQL 或 Redis。HCI 和 Virt 版本绑定资源类型仅支持虚拟机，网卡类型为扁平。
...
- 资源 ID：安全组的全局唯一标识符；
- 资源名称：安全组的名称；
- 绑定资源类型：安全组支持绑定的资源类型，包括虚拟机、负载均衡、 NAT 网关、网卡、对象存储、文件存储、MySQL 及 Redis；
...
支持 MySQL 绑定安全组，如下图所示：

![mysql_bindsg](../images/userguide/mysql_bindsg.png)

- 资源 ID：安全组的全局唯一标识符；
- 资源名称：安全组的名称；
- 绑定资源类型：安全组支持绑定的资源类型，包括虚拟机、负载均衡、 NAT 网关、网卡、对象存储、文件存储、MySQL 及 Redis；
- 资源信息：资源信息列表展示 MySQL 资源列表，可多选。

支持 Redis 绑定安全组，如下图所示：

![redis_bindsg](../images/userguide/redis_bindsg.png)

- 资源 ID：安全组的全局唯一标识符；
- 资源名称：安全组的名称；
- 绑定资源类型：安全组支持绑定的资源类型，包括虚拟机、负载均衡、 NAT 网关、网卡、对象存储、文件存储、MySQL 及 Redis；
- 资源信息：资源信息列表展示 Redis 资源列表，可多选。

HCI 和 Virt 版本绑定资源类型仅支持虚拟机和网卡，如下图所示：

![vm_bindsghci](../images/userguide/vm_bindsghci.png)
![nic_bindsghci](../images/userguide/nic_bindsghci.png)

- 资源 ID：安全组的全局唯一标识符；
- 资源名称：安全组的名称；
- 绑定资源类型：安全组支持绑定的资源类型，仅包括虚拟机；
- 资源信息：资源信息列表展示虚拟机资源列表，可多选。
...
- IP 组备注：当前IP组的备注；
- 项目组：可绑定当前租户下创建的项目组，HCI 和 Virt 版本不支持选择项目组。
...
- 项目组：IP 组绑定的项目组，HCI 和 Virt 版本不支持查看项目组；
- 创建时间：IP 组的创建时间。
...
- 项目组：端口组绑定的项目组，HCI 和 Virt 版本不支持查看项目组；
- 创建时间：端口组的创建时间。
